...
class InterpretabilityAnalyzer:
    """Analyseur d'interprétabilité pour les modèles FTT."""
    ...
    def analyze_and_save(
        ...
        # Calcul de l'importance des features via le token CLS
        model.eval()
        with torch.no_grad():
            if hasattr(model, 'get_cls_importance'):
                cls_importance = model.get_cls_importance(X['test'][0], X['test'][1], feature_names)
            else:
                raise NotImplementedError("Le modèle doit implémenter get_cls_importance() pour l'analyse d'interprétabilité")
        ...
        # Sauvegarde des résultats bruts d'interprétabilité
        interpretability_results = self._save_interpretability(model_name, seed, feature_names, cls_importance)
    ...
    def _save_interpretability(self, model_name: str, seed: int, feature_names: List[str], cls_importance: Dict[str, float]) -> Dict:
        """Sauvegarde les résultats bruts d'interprétabilité."""
        # Tri des features par importance pour le top 10
        sorted_importance = sorted(cls_importance.items(), key=lambda x: x[1], reverse=True)[:10]
        ...
        data = {
            ...
            'top_features': [{'name': name, 'importance': float(score)} for name, score in sorted_importance]
        }
    ...
    def _generate_and_save_visualizations(self, model: torch.nn.Module, X: Dict, feature_names: List[str], model_name: str, seed: int, cls_importance: Dict[str, float]):
        """Génère et sauvegarde les visualisations d'interprétabilité."""
